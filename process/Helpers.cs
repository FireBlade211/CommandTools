...
namespace process
{
    ...
    {
        public static string GetDisplayName(this Process process)
        {
            try
            {
        ...
        public static bool IsPrimaryProcess(this Process proc)
        {
            int? parentPid = proc.GetParentProcessId();

            // PID 0 (System Idle) or null means no real parent
            return parentPid == null || parentPid <= 0;
        }


        private static class CommandLineHelper
        {
                                        ...
                                        pbiInfo.PebBaseAddress, out var pebInfo))
                                    {
                                            ...
                                            hProcess, pebInfo.ProcessParameters, out var ruppInfo))
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                {
                                                    commandLine = Marshal.PtrToStringUni(memCL) ?? "[Error]";
                                                    rc = 0;
                                                }
                                                else
                                                {
        ...
        {
            CommandLineHelper.Retrieve(process, out string? cmdLine);
